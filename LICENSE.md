IMPERIUM-MD PROPRIETARY LICENSE
ALL RIGHTS RESERVED

────────────────────────────────────────────────────────────
SECTION 1 — OWNERSHIP AND COPYRIGHT
────────────────────────────────────────────────────────────

Copyright (c) 2026 mudau_t

IMPERIUM-MD, including but not limited to its source code, structure,
logic, command systems, configuration files, documentation, branding,
name, design, and behavior (collectively referred to as the "Software"),
is the exclusive intellectual property of the owner known as:

OWNER: mudau_t

The Software is NOT open-source. Ownership is not transferred under
any circumstances. All rights not explicitly granted are fully reserved
by the owner.

Any reference to “user” or “you” refers to any individual, group,
organization, or entity accessing or using the Software.

────────────────────────────────────────────────────────────
SECTION 2 — GRANT OF LIMITED PERMISSION
────────────────────────────────────────────────────────────

Permission is granted ONLY to:

• Run the Software in its original, unmodified form
• Use the Software for personal or explicitly authorized purposes
• Operate the Software as provided by the owner

This permission is:
• Non-exclusive
• Non-transferable
• Revocable at any time
• Limited strictly to usage only

NO ownership rights, modification rights, or redistribution rights
are granted by this license.

────────────────────────────────────────────────────────────
SECTION 3 — STRICT PROHIBITIONS
────────────────────────────────────────────────────────────

You are STRICTLY FORBIDDEN from performing any of the following actions,
whether partially or fully, directly or indirectly:

1. Modifying, editing, altering, refactoring, or rewriting the source code
2. Adding, removing, or changing features or commands
3. Copying any portion of the code, logic, structure, or design
4. Reverse engineering, decompiling, or disassembling the Software
5. Rebranding, renaming, or changing the identity of the Software
6. Claiming authorship or ownership of the Software
7. Removing or altering credits, headers, or license text
8. Redistributing or reuploading the Software to any platform
9. Forking the repository or creating derivative works
10. Selling, renting, leasing, sublicensing, or monetizing the Software
11. Using the Software for commercial services without written permission
12. Training AI models using the Software or its code
13. Using the Software as a base for another project
14. Circumventing license checks or ownership controls

Any of the above actions constitute a **license violation**.

────────────────────────────────────────────────────────────
SECTION 4 — NO DERIVATIVE WORKS
────────────────────────────────────────────────────────────

Derivative works are NOT permitted.

A derivative work includes, but is not limited to:
• Modified versions
• Renamed versions
• Feature-added versions
• Obfuscated or minified versions
• Translated versions
• Recompiled or repackaged versions

The Software must remain EXACTLY as distributed by the owner.

────────────────────────────────────────────────────────────
SECTION 5 — DISTRIBUTION AND HOSTING
────────────────────────────────────────────────────────────

You MAY NOT:
• Share the Software publicly or privately
• Upload the Software to GitHub, GitLab, or any code host
• Provide copies to third parties
• Include the Software in bundles or packages

Only the owner (mudau_t) has the right to distribute the Software.

────────────────────────────────────────────────────────────
SECTION 6 — COMMERCIAL USE
────────────────────────────────────────────────────────────

Commercial use is STRICTLY PROHIBITED unless explicit written permission
is granted by the owner.

Commercial use includes:
• Paid services
• Subscription-based bots
• Donation-gated access
• Ads or monetization tied to the Software
• Business or enterprise usage

Unauthorized commercial use is a serious violation.

────────────────────────────────────────────────────────────
SECTION 7 — SECURITY AND TAMPERING
────────────────────────────────────────────────────────────

Any attempt to:
• Bypass restrictions
• Disable safeguards
• Inject unauthorized code
• Exploit internal logic

will be considered malicious intent and a direct breach of this license.

────────────────────────────────────────────────────────────
SECTION 8 — TERMINATION OF RIGHTS
────────────────────────────────────────────────────────────

This license is automatically TERMINATED if you violate any term.

Upon termination, you MUST:
• Stop using the Software immediately
• Delete all copies of the Software
• Remove it from all systems and devices

No notice is required from the owner for termination.

────────────────────────────────────────────────────────────
SECTION 9 — DISCLAIMER OF WARRANTY
────────────────────────────────────────────────────────────

THE SOFTWARE IS PROVIDED "AS IS".

THE OWNER MAKES NO WARRANTIES, EXPRESS OR IMPLIED, INCLUDING BUT NOT
LIMITED TO MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, OR
NON-INFRINGEMENT.

THE OWNER IS NOT RESPONSIBLE FOR:
• WhatsApp bans
• Account restrictions
• Data loss
• Misuse by users
• Legal consequences of usage

USE AT YOUR OWN RISK.

────────────────────────────────────────────────────────────
SECTION 10 — LIMITATION OF LIABILITY
────────────────────────────────────────────────────────────

IN NO EVENT SHALL THE OWNER BE LIABLE FOR ANY CLAIM, DAMAGES, OR OTHER
LIABILITY, WHETHER IN AN ACTION OF CONTRACT, TORT, OR OTHERWISE, ARISING
FROM, OUT OF, OR IN CONNECTION WITH THE SOFTWARE OR ITS USE.

────────────────────────────────────────────────────────────
SECTION 11 — LEGAL ENFORCEMENT
────────────────────────────────────────────────────────────

Violations may result in:
• DMCA takedown requests
• Platform enforcement actions
• Account suspension or bans
• Legal claims where applicable

The owner reserves the right to enforce this license to the fullest
extent permitted by law.

────────────────────────────────────────────────────────────
SECTION 12 — GOVERNING AUTHORITY
────────────────────────────────────────────────────────────

This license shall be governed and interpreted according to applicable
laws, without regard to conflict of law principles.

────────────────────────────────────────────────────────────
SECTION 13 — FINAL STATEMENT
────────────────────────────────────────────────────────────

IMPERIUM-MD is a protected, proprietary project.

Respect the ownership.
Respect the license.
Respect the crown.

OWNER: mudau_t
PROJECT: IMPERIUM-MD

© 2026 mudau_t — ALL RIGHTS RESERVED
